...
const handleHttp = ({ description, email, screenshot, config }) => {
  const { destination } = config;

  return sendHttp(description, email, screenshot, destination);
};
...
const handleMail = ({ description, email, screenshot, additionalInfo, config }) => new Promise((resolve, reject) => {
  ...
  Email.send({
    ...
    Body: `<pre>${JSON.stringify(data, 2, 2)}</pre>`,
    Attachments: [
      {
        name: filename,
        data: screenshot
...
export const onOk = async ({ publishConfig, description, email, screenshot, additionalInfo = {} }) => {
