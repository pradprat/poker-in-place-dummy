...
class Room extends EventEmitter {
  ...
  handleProtooConnection({ peerId, consume, protooWebSocketTransport }) {
    ...
    peer.on("close", () => {
      ...
      if (peer.data.joined) {
        ...
        const participatingWhisper = this._getActiveWhispers().find(
          (w) => w.fromPeerId === peer.id || w.toPeerId === peer.id
        );
        for (const otherPeer of otherPeers) {
          otherPeer.notify("peerClosed", { peerId: peer.id }).catch(() => {});
          if (participatingWhisper) {
            otherPeer
              .notify("whisperEnded", {
                fromPeerId: participatingWhisper.fromPeerId,
                toPeerId: participatingWhisper.toPeerId,
              })
              .catch(() => {});
          }
        }
        this._activeWhispers = this._activeWhispers.filter(
          (w) => w !== participatingWhisper
        );
      }
  ...
  async createBroadcaster({ id, displayName, device = {}, rtpCapabilities }) {
    ...
    return { peers: peerInfos, whispers: this._getActiveWhispers() };
  }
  ...
  async _handleProtooRequest(peer, request, accept, reject) {
    switch (request.method) {
      ...
      case "join": {
        ...
        const whisperInfos = this._getActiveWhispers();

        // Notify all the peers, whispers to the new peer
        accept({ peers: peerInfos, whispers: whisperInfos });
      ...
      case "startWhisper": {
        ...
        const activeFromWhisper = this._getActiveWhispers(
          (w) => w.fromPeerId === peer.id || w.toPeerId === peer.id
        );

        // Notify the peers of removing it
        const otherPeers = this._getJoinedPeers({ excludePeer: peer });

        if (activeFromWhisper) {
          for (const otherPeer of otherPeers) {
            otherPeer
              .notify("whisperEnded", {
                fromPeerId: activeFromWhisper.fromPeerId,
                toPeerId: activeFromWhisper.toPeerId,
              })
              .catch(() => {});
          }
          this._activeWhispers = this._activeWhispers.filter(
            (w) => w !== activeFromWhisper
          );
        }

        const activeToWhisper = this._getActiveWhispers().find(
          (w) => w.fromPeerId === toPeerId || w.toPeerId === toPeerId
        );

        if (activeToWhisper) {
          reject(403, "operation NOT allowed - active whisper");
          return;
        }

		this._activeWhispers.push({ fromPeerId: peer.id, toPeerId });
		
        for (const otherPeer of otherPeers) {
          otherPeer
            .notify("whisperStarted", {
              fromPeerId: peer.id,
              toPeerId,
            })
            .catch(() => {});
      ...
      case "endWhisper": {
        ...
        const activeWhisper = this._activeWhispers.find(
          (w) =>
            (w.fromPeerId === peer.id && w.toPeerId === toPeerId) ||
            (w.toPeerId === peer.id && w.fromPeerId === toPeerId)
        );
        ...
        if (activeWhisper) {
          for (const otherPeer of otherPeers) {
            otherPeer
              .notify("whisperEnded", {
                fromPeerId: activeWhisper.fromPeerId,
                toPeerId: activeWhisper.toPeerId,
              })
              .catch(() => {});
          }
          this._activeWhispers = this._activeWhispers.filter(
   ...
   * Helper to get the list of joined protoo peers.
   */
  _getActiveWhispers({ excludePeer = undefined } = {}) {
	const peers = this._getJoinedPeers();
    return excludePeer
      ? this._activeWhispers.filter(
          (whisper) =>
            whisper.toPeerId !== excludePeer &&
            whisper.fromPeerId !== excludePeer &&
            peers.find((p) => p.id === whisper.fromPeerId) &&
            peers.find((p) => p.id === whisper.toPeerId)
        )
      : this._activeWhispers.slice(0);
  }
